<!DOCTYPE html>
<html lang="ru">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta
        name="description"
        content="Разработка IT-инструментов для ваших проектов" />
    <link rel="shortcut icon" href="/img/logo-favicon.svg" type="image/x-icon" />
    <link
        rel="stylesheet"
        href="https://fonts.googleapis.com/css2?family=Montserrat:wght@300;400;500;600;700;900&display=swap" />
    <title>EndPoint - Разработка IT-инструментов для ваших проектов</title>

    <link rel="stylesheet" href="/css/style.min.css">
</head>

<body>
    <div class="wrapper">

        <?php
        include_once('header.php');
        ?>
        <div class="wrapper__main">
            <div class="breadcrumbs">
                <div class="breadcrumbs__container container">
                    <ul class="breadcrumbs__list">
                        <li class="breadcrumbs__item">
                            <a href="/" class="breadcrumbs__item-link">
                                Главная
                            </a>
                        </li>
                        <li class="breadcrumbs__item">
                            Кейсы
                        </li>
                    </ul>
                </div>

            </div>

            <div class="cases cases__page">
                <div class="cases__container container">
                    <div class="cases__top">
                        <h2 class="cases__title title title_h2">Кейсы</h2>
                    </div>
                    <ul class="cases__list">
                        <li class="cases__item">
                            <a href="/cases/1" class="cases__item-link">
                                <h4 class="cases__item-link-title">Удобная корзина</h4>
                                <p class="cases__item-link-description">Модуль для 1С-Битрикс24 для просмотра корзины с иерархией папок</p>
                                <picture>
                                    <source srcset="/img/case-1.webp" type="image/webp">
                                    <img class="cases__item-link-img" src="/img/case-1.png" alt="">
                                </picture>
                            </a>
                        </li>
                        <li class="cases__item">
                            <a href="/cases/2" class="cases__item-link">
                                <h4 class="cases__item-link-title">Яндекс Заправки - управление</h4>
                                <p class="cases__item-link-description">Приложение для Битрикс24 для управление доступом сотрудников к Яндекс Заправкам</p>
                                <picture>
                                    <source srcset="/img/case-2.webp" type="image/webp">
                                    <img class="cases__item-link-img" src="/img/case-2.png" alt="">
                                </picture>
                            </a>
                        </li>
                        <li class="cases__item">
                            <a href="/cases/3" class="cases__item-link">
                                <h4 class="cases__item-link-title">Приветствие новому сотруднику</h4>
                                <p class="cases__item-link-description">Приложение Битрикс24 для формирования приветствия для всех новых сотрудников компании</p>
                                <picture>
                                    <source srcset="/img/case-3.webp" type="image/webp">
                                    <img class="cases__item-link-img" src="/img/case-3.png" alt="">
                                </picture>
                            </a>
                        </li>
                        <li class="cases__item">
                            <a href="/cases/4" class="cases__item-link">
                                <h4 class="cases__item-link-title">Удобная корзина</h4>
                                <p class="cases__item-link-description">Модуль для 1С-Битрикс24 для просмотра корзины с иерархией папок</p>
                                <picture>
                                    <source srcset="/img/case-4.webp" type="image/webp">
                                    <img class="cases__item-link-img" src="/img/case-4.png" alt="">
                                </picture>
                            </a>
                        </li>
                        <li class="cases__item">
                            <a href="/cases/4" class="cases__item-link">
                                <h4 class="cases__item-link-title">Удобная корзина</h4>
                                <p class="cases__item-link-description">Модуль для 1С-Битрикс24 для просмотра корзины с иерархией папок</p>
                                <picture>
                                    <source srcset="/img/case-4.webp" type="image/webp">
                                    <img class="cases__item-link-img" src="/img/case-4.png" alt="">
                                </picture>
                            </a>
                        </li>
                        <li class="cases__item">
                            <a href="/cases/4" class="cases__item-link">
                                <h4 class="cases__item-link-title">Удобная корзина</h4>
                                <p class="cases__item-link-description">Модуль для 1С-Битрикс24 для просмотра корзины с иерархией папок</p>
                                <picture>
                                    <source srcset="/img/case-4.webp" type="image/webp">
                                    <img class="cases__item-link-img" src="/img/case-4.png" alt="">
                                </picture>
                            </a>
                        </li>
                        <li class="cases__item">
                            <a href="/cases/4" class="cases__item-link">
                                <h4 class="cases__item-link-title">Удобная корзина</h4>
                                <p class="cases__item-link-description">Модуль для 1С-Битрикс24 для просмотра корзины с иерархией папок</p>
                                <picture>
                                    <source srcset="/img/case-4.webp" type="image/webp">
                                    <img class="cases__item-link-img" src="/img/case-4.png" alt="">
                                </picture>
                            </a>
                        </li>
                        <li class="cases__item">
                            <a href="/cases/4" class="cases__item-link">
                                <h4 class="cases__item-link-title">Удобная корзина</h4>
                                <p class="cases__item-link-description">Модуль для 1С-Битрикс24 для просмотра корзины с иерархией папок</p>
                                <picture>
                                    <source srcset="/img/case-4.webp" type="image/webp">
                                    <img class="cases__item-link-img" src="/img/case-4.png" alt="">
                                </picture>
                            </a>
                        </li>
                        <li class="cases__item">
                            <a href="/cases/4" class="cases__item-link">
                                <h4 class="cases__item-link-title">Удобная корзина</h4>
                                <p class="cases__item-link-description">Модуль для 1С-Битрикс24 для просмотра корзины с иерархией папок</p>
                                <picture>
                                    <source srcset="/img/case-4.webp" type="image/webp">
                                    <img class="cases__item-link-img" src="/img/case-4.png" alt="">
                                </picture>
                            </a>
                        </li>

                    </ul>
                </div>
            </div>

            <?php

            include_once('expertise.php');
            include_once('services.php');
            include_once('contacts.php');
            ?>
        </div>


        <?php

        include_once('footer.php');

        ?>

    </div>
</body>

</html>